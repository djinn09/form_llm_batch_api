...
class BatchRequest(BaseModel):
    ...
    custom_id: str = Field(
        ...,
        description=(
            "A unique identifier for the request to match it with the " "response."
        ),
    )
    method: str = Field(
        "POST",
    ...
    url: str = Field(
        "/v1/chat/completions", description="The API endpoint for the request."
    )
...
class ComparisonResult(BaseModel):
    ...
    document_field_id: str = Field(
        ...,
        description=(
            "The custom_id from the original request, linking back to the "
            "document field."
        ),
    )
    ...
    status: str = Field(
        ...,
        description=(
            "The outcome of the processing, e.g., 'Match Found', 'Mismatch', "
            "'Error'."
        ),
    )
    error_message: Optional[str] = Field(
        None, description="Any error message encountered during processing."
    )
